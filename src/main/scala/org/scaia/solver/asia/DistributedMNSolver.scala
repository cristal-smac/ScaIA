...
import org.scaia.actor.{Result, Start, Supervisor}
...
/**
  ...
  *
  */
class DistributedMNSolver(pb : IAProblem, system: ActorSystem, restricted: Boolean, rule: SocialRule) extends ASIASolver(pb){
  ...
  override def solve() : Matching = {
    ...
    val supervisor = system.actorOf(Props(classOf[Supervisor], pb, restricted, rule), name = "supervisor"+DistributedMNSolver.id)
